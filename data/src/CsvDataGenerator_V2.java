...
public class CsvDataGenerator_V2 {
    ...
    public static void main(String[] args) {
        ...
        // 生成data1.csv - 1000条数据
        generateData1(1000, data1FilePath);
    ...
    private static String modifyRecord(String originalRecord) {
        String[] fields = originalRecord.split(COMMA);

        // 修改CURR_BAL (索引16)
        fields[16] = quoteField(String.format("%.2f", random.nextDouble() * 1000000));

        // 修改INVM_TRM_VALUE (索引18)
        fields[18] = quoteField(String.format("%.2f", random.nextDouble() * 1000000));

        // 修改NRDA_THRSHLD_AMT (索引65)
        fields[65] = quoteField(String.format("%.2f", 1000 + random.nextDouble() * 99000));
